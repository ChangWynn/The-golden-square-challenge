RSpec.describe "doubles" do
  ...
  it "sets up a double with methods" do
    ...
    # allow(fake_object).to receive(:speak) {"Meow"}
    # allow(fake_object).to receive(:count_ears) {2}
    # allow(fake_object).to receive(:count_legs) {4}
  ...
  it "sets up a double with methods that take arguments" do
    # Set up your doubles here
    fake_object = double :FakeObject
    allow(fake_object).to receive(:speak).with("Jess") {"Meow, Jess"}
  ...
  it "sets up doubles that expect to be called" do
    ...
    # Write an expectation below that the method "speak" is called with the argument "Steve"
  ...
  it "creates a double for a specific case" do
    ...
    # ...
    allow(fake_diary).to receive(:count_entries) {2}
